...
board.on('update', function() {
  ...
  for (i = 0; i!= maxatoms; i++) {
    if (o == p[0]) {
      if ((p[1].position - p[0].position< 0.025)) {
    ...
    positionsInput.value = JSON.stringify(positions);
      ...
      if ((p[maxatoms - 1].position - p[maxatoms - 2].position< 0.025)) {
      ...
      if ((p[i].position - p[i - 1].position< 0.025)) {
      ...
      if ((p[i + 1].position - p[i].position< 0.025)) {
        p[i].position = p[i + 1].position - 0.05;
        p[i].prepareUpdate().update(true).updateRenderer();
        positions[i] = p[i].X().toFixed(3);
        positionsInput.value = JSON.stringify(positions);
...
//----------------
if (design != 1) {
  for (i = 0; (i!= maxatoms); i++) {    p[i].setAttribute({fixed: true});  }
} 

if (design==1){

     let temp=[];  for (i = 0; (i!= maxatoms); i++) {temp[i]="0";oxid[i]="0"};
      stateInput.value=JSON.stringify(temp);
