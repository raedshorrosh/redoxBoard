...
// ------------------------------------------------------------------
// Visual marking after Moodle/STACK has submitted the response.
// STACK clones the readonly state to the JSXGraph input reference,
// so no feedback span id or feedback-content lookup is needed.
// ------------------------------------------------------------------
function markGraph() {
  if (answered) {
    return;
  ...
  for (i = 0; isless(i, maxatoms); i++) {
    if (parseFloat(p[i].name) == parseFloat(oxid_ans[i])) {
      p[i].name = '<span style="font-size: 1em; color: green;">' + p[i].name + ' ✔</span>';
    } else {
      p[i].name = '<span style="font-size: 1em; color: red;">' + p[i].name + ' ❌</span>';
    }
  }
...
// Same one-time trigger pattern used in levels3-new.js.
// No polling is required: when STACK reloads the submitted question,
// statesRef is already readonly.
if (stateInput && stateInput.hasAttribute('readonly')) {
  markGraph();
}
